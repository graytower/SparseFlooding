...
/*
 * XPath: /frr-interface:lib/interface/frr-isisd:isis/no-flood
 */
DEFPY_YANG(isis_no_flood,
           isis_no_flood_cmd,
           "[no] isis no-flood",
           NO_STR
           PROTO_HELP
           "Do not flood IS-IS LSPs on this interface\n")
{
    nb_cli_enqueue_change(vty, "./frr-isisd:isis/no-flood",
                          NB_OP_MODIFY, no ? "false" : "true");

    return nb_cli_apply_changes(vty, NULL);
}

void cli_show_ip_isis_no_flood(struct vty *vty, const struct lyd_node *dnode,
                               bool show_defaults)
{
    if (!yang_dnode_get_bool(dnode, NULL))
        vty_out(vty, " no");
    vty_out(vty, " isis no-flood\n");
}
...
void isis_cli_init(void)
{
	...
	/* self-implemented: PARTIAL FLOODING*/
	install_element(INTERFACE_NODE, &isis_no_flood_cmd);

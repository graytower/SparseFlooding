...
uint8_t is_flood_disabled = 0;

    if (oi->flood_control == OSPF_FLOOD_DISABLED) is_flood_disabled = 1;
    if (params && params->flood_control == OSPF_FLOOD_DISABLED) is_flood_disabled = 1;
	...
	if (IS_DEBUG_OSPF_EVENT){
		...
		if(params)
			zlog_debug("  Default flood_control=%d", params->flood_control);

	}
	
	if (IS_DEBUG_OSPF_EVENT)
        zlog_debug("DEBUG-CHECK: Int %s, flood Config_val=%d", 
                   IF_NAME(oi), oi->flood_control);
	...
	if (is_flood_disabled) {
		for (rn = route_top(oi->nbrs); rn; rn = route_next(rn)) {
			...
			if (onbr->state < NSM_Full){
				if (IS_DEBUG_OSPF_EVENT)
					zlog_debug("Flood-Control: Allow sync LSA[%s] to %pI4 (State: %s)", 
												dump_lsa_key(lsa),
												&onbr->router_id,
				...
				if (IS_DEBUG_OSPF_EVENT) {
                    zlog_debug("Flood-Control: Block LSA[%s] to Full neighbor %pI4 on %s", 
                               dump_lsa_key(lsa),
                               &onbr->router_id,
                               IF_NAME(oi));
                }

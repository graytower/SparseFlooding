...
/* self-implemented: PARTIAL FLOODING*/
/* 融合版命令实现 */
DEFUN (ip_ospf_no_flood,
       ip_ospf_no_flood_cmd,
       "ip ospf no-flood",
       "IP Information\n"
       "OSPF interface commands\n"
       "Disable LSA flooding on this interface (Tree-based flooding only)\n")
{
    ...
    /* 1. 【持久化层】保存配置
     * 用户输入了这个命令，意味着他想"禁止洪泛" 
     */
    params = IF_DEF_PARAMS(ifp);
    if (params->flood_control != OSPF_FLOOD_DISABLED) {
        params->flood_control = OSPF_FLOOD_DISABLED;
    }

    /* 2. 【运行时层】立即应用到所有 OSPF 实例 */
    for (rn = route_top(IF_OIFS(ifp)); rn; rn = route_next(rn)) {
        oi = rn->info;
        if (oi == NULL) continue;

        /* 修改运行态 */
        oi->flood_control = OSPF_FLOOD_DISABLED;
        
        if (IS_DEBUG_OSPF_EVENT) {
            zlog_debug("OSPF: Flooding DISABLED (no-flood) on interface %s", ifp->name);
        }
    }
...
/* * 命令: no ip ospf no-flood
 * 含义: 取消"禁止洪泛" (恢复为 NORMAL)
 */
DEFUN (no_ip_ospf_no_flood,
       no_ip_ospf_no_flood_cmd,
       "no ip ospf no-flood",
       NO_STR
       "IP Information\n"
       "OSPF interface commands\n"
       "Disable LSA flooding on this interface\n")
{
    VTY_DECLVAR_CONTEXT(interface, ifp);
    struct ospf_if_params *params;
    struct route_node *rn;
    ...
    /* 1. 恢复配置默认值 (NORMAL) */
    params = IF_DEF_PARAMS(ifp);
    params->flood_control = OSPF_FLOOD_NORMAL;

    /* 2. 恢复运行态 */
    for (rn = route_top(IF_OIFS(ifp)); rn; rn = route_next(rn)) {
        oi = rn->info;
        if (oi) {
            oi->flood_control = OSPF_FLOOD_NORMAL;
            if (IS_DEBUG_OSPF_EVENT) {
                zlog_debug("OSPF: Flooding RESTORED (normal) on interface %s", ifp->name);
            }
        }
    }
...
static void ospf_vty_if_init(void)
{
	...
	/* self-implemented: PARTIAL FLOODING*/
	install_element(INTERFACE_NODE, &ip_ospf_no_flood_cmd);
	install_element(INTERFACE_NODE, &no_ip_ospf_no_flood_cmd);
}
